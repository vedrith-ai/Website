...
export default function KundaliBirthForm({ onSubmit, loading }: Props) {
  ...
  const handleSubmit = useCallback((e: FormEvent) => {
    ...
    if (!name.trim()) { setFormError(tr('Please enter the full name.','ದಯವಿಟ್ಟು ಪೂರ್ಣ ಹೆಸರನ್ನು ನಮೂದಿಸಿ.')); return }
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-5">
      <div><label className={lbl}>{tr('Full Name','ಪೂರ್ಣ ಹೆಸರು')}</label>
        <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder={tr('As it appears on the chart','ಕುಂಡಲಿಯಲ್ಲಿ ಕಾಣಿಸಬೇಕಾದ ಹೆಸರು')} className={inp} required /></div>

      <div><p className={lbl}>{tr('Gender','ಲಿಂಗ')}</p>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div><label className={lbl}>{tr('Date of Birth','ಜನ್ಮ ದಿನಾಂಕ')}</label>
        ...
        <div><label className={lbl}>{tr('Time of Birth','ಜನ್ಮ ಸಮಯ')}</label>
      ...
      <div><label className={lbl}>{tr('Birth Place','ಜನ್ಮ ಸ್ಥಳ')}</label>
        <div className="flex gap-2">
          <input type="text" value={placeQuery} onChange={e => setPlaceQuery(e.target.value)}
            ...
            placeholder={tr('Search city or town…','ನಗರ ಅಥವಾ ಊರನ್ನು ಹುಡುಕಿ…')} className={`${inp} flex-1`} />
          <button type="button" onClick={handlePlaceSearch} disabled={searching || !placeQuery.trim()}
            ...
            {searching ? '…' : tr('Search','ಹುಡುಕಿ')}</button>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div><label className={lbl}>{tr('Latitude','ಅಕ್ಷಾಂಶ')}</label><input type="number" step="0.000001" min="-90" max="90" value={lat} onChange={e => setLat(e.target.value)} placeholder="12.9716" className={inp} required /></div>
        <div><label className={lbl}>{tr('Longitude','ರೇಖಾಂಶ')}</label><input type="number" step="0.000001" min="-180" max="180" value={lng} onChange={e => setLng(e.target.value)} placeholder="77.5946" className={inp} required /></div>
      </div>

      <div><label className={lbl}>{tr('Timezone','ಸಮಯ ವಲಯ')}</label>
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => setTimezone(getBrowserTimezone())} className="px-3 py-3 bg-navy-800 border border-white/10 text-cream-100/60 hover:text-gold-400 transition-colors font-sans text-xs whitespace-nowrap">{tr('Detect','ಪತ್ತೆಹಚ್ಚಿ')}</button>
        </div>
        <p className="mt-1 text-[0.65rem] text-cream-100/30 font-sans">IANA format e.g. &quot;Asia/Kolkata&quot;</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div><label className={lbl}>{tr('Ayanamsha','ಅಯನಾಂಶ')}</label>
        ...
        <div><label className={lbl}>{tr('House System','ಭಾವ ಪದ್ಧತಿ')}</label>
      ...
      <button type="submit" disabled={loading} className={`w-full py-4 bg-gold-500/20 border border-gold-500/40 text-gold-400 font-sans text-sm tracking-[0.18em] uppercase hover:bg-gold-500/30 transition-colors ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}>
        {loading ? tr('Generating Kundali…','ಕುಂಡಲಿ ರಚಿಸಲಾಗುತ್ತಿದೆ…') : tr('Generate Kundali Chart','ಕುಂಡಲಿ ಚಾರ್ಟ್ ರಚಿಸಿ')}
      </button>
    </form>
  )
}
