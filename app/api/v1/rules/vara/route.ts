// GET /api/v1/rules/vara?key=Monday&lang=en&region=Karnataka
  ...
  try {
    ...
    if (!key) {
      return NextResponse.json({
        success: false,
        error: { code: 'MISSING_KEY', message: 'key parameter required (e.g. ?key=Monday)' }
      }, { status: 400 })
    }

    const result = await explainElement('vara', key, { lang, useAI })
    if (!result) {
      return NextResponse.json({
        success: false,
        error: { code: 'NOT_FOUND', message: `Vara "${key}" not found` }
      }, { status: 404 })
    }

    const applicableRules = getRulesForElement('vara', key)
    ...
    console.error('[/api/v1/rules/vara]', err)
    return NextResponse.json({
